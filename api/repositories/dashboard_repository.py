...
class DashboardRepository:
    ...
    def get_summary(self) -> dict:
        ...
        analyzed_condition = IOC.vt_estado == "analizado"

        malicious = self.db.scalar(
            select(func.count(IOC.id)).where(
                and_(
                    analyzed_condition,
                    func.coalesce(IOC.vt_malicious, 0) > 0,
                )
            )
        ) or 0
        ...
        suspicious = self.db.scalar(
            select(func.count(IOC.id)).where(
                and_(
                    analyzed_condition,
                    func.coalesce(IOC.vt_malicious, 0) == 0,
                    func.coalesce(IOC.vt_suspicious, 0) > 0,
                )
            )
        ) or 0
    ...
    def get_types(self) -> list[dict]:
        statement = (
            select(
                func.coalesce(IOC.tipo, "unknown").label("label"),
                func.count(IOC.id).label("total"),
            )
            .group_by(func.coalesce(IOC.tipo, "unknown"))
    ...
    def get_sources(self, limit: int = 8) -> list[dict]:
        statement = (
            select(
                func.coalesce(
                    IOC.fuente,
    ...
    def get_campaigns(self, limit: int = 8) -> list[dict]:
        statement = (
            select(
    ...
    def get_malware(self, limit: int = 8) -> list[dict]:
        statement = (
            select(
    ...
    def get_monthly_activity(
        ...
        statement = (
            ...
            .where(IOC.fecha_creacion.is_not(None))
            .group_by(month)
            .order_by(month.desc())
            .limit(limit)
        )
        ...
        rows = list(
            self.db.execute(statement).mappings().all()
        )
